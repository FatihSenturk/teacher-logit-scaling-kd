...
def _tracker_config(value):
    """Convert argparse/config values into tracker-safe plain Python types."""
    if isinstance(value, argparse.Namespace):
        return {key: _tracker_config(item) for key, item in vars(value).items()}
    if isinstance(value, dict):
        return {str(key): _tracker_config(item) for key, item in value.items()}
    if isinstance(value, (list, tuple, set)):
        return [_tracker_config(item) for item in value]
    if isinstance(value, (torch.device, os.PathLike)):
        return str(value)
...
def init_environment(args):
    ...
    if args.model_name in pretrained_models:
        ...
        try:
            model = timm.create_model(args.model_name, pretrained=args.pretrained_timm, num_classes=args.num_classes)
        except Exception:
    ...
    if args.pretrained_local is not None:
        ...
        start_epoch = 0
    # Optional override: load pretrained weights but start training loop from a custom epoch
    # (e.g., 0 for finetuning instead of resume).
    if hasattr(args, "start_epoch_override") and args.start_epoch_override is not None:
        start_epoch = int(args.start_epoch_override)
    #测试改进模型
    ...
    if args.schedule_name =='CosineAnnealingLR':
        ...
        schedule = torch.optim.lr_scheduler.ExponentialLR(
            optimizer, gamma=float(getattr(args, "gamma", 0.98))
        )
    if args.pretrained_timm==False and args.pretrained_local is not None:
        ...
        if "schedule" in checkpoint:
            ...
            schedule.step(start_epoch)

    if args.use_amp:
        # SAM performs a two-step update and calls the wrapped optimizer directly.
        # Keep autocast enabled for memory/speed, but avoid GradScaler with SAM so
        # the perturbation step never sees scaled or overflowed gradients.
        scaler = torch.amp.GradScaler(enabled=not getattr(args, "use_sam", False))
        amp_context = torch.amp.autocast
        if args.pretrained_local is not None and "scaler" in checkpoint:
...
def main(args, wandb):
    ...
    best_metric_value = 0.0 if args.metric_direction=='up' else 1000.0
    ...
    for epoch in epochrange:
        #train
        if train_loader is not None:
            print(f"epoch:{epoch},completed:{epoch + 1}/{args.max_epochs}")
        ...
        if  (epoch + 1) % args.eval_freq == 0 and val_loader is not None: 
            metric_value = _to_float_metric(validloop(wandb,args, model,epoch, val_loader,embeddings))
            if args.metric_direction == 'up':
                metric_delta = metric_value - best_metric_value
            else:
                metric_delta = best_metric_value - metric_value
            save_model = metric_delta > 0
            ...
            print(
                f"epoch:{epoch},completed:{epoch + 1}/{args.max_epochs},"
                f"best_{args.metric_name}:{best_metric_value:.4f},{args.metric_name}:{metric_value:.4f}"
            )
...
if __name__ == "__main__":
    ...
    for cfg_file in args.c.split(','):
        ...
        if args.resume is not None:
            args.pretrained_local = args.resume
        if not hasattr(args, "use_swanlab"):
            args.use_swanlab = False
        if not hasattr(args, "use_wandb"):
            args.use_wandb = False
        log_root = getattr(args, "log_root", r'd:lg\logs')
        log_time = "{0:%Y-%m-%d-%H-%M-%S}".format(datetime.now())
        ...
        tracker_config = _tracker_config(args)
        if args.use_swanlab:
            import swanlab

            swanlab.init(
                project=getattr(args, "swanlab_project", f'FGVC-{args.dataset_name}-{args.num_classes}'),
                workspace=getattr(args, "swanlab_workspace", None),
                experiment_name=getattr(args, "swanlab_experiment_name", log_time),
                description=getattr(args, "swanlab_description", None),
                tags=getattr(args, "swanlab_tags", None),
                config=tracker_config,
                logdir=getattr(args, "swanlab_logdir", log_root),
                mode=getattr(args, "swanlab_mode", "cloud"),
            )
        run_logger = _NoOpLogger()
        if args.use_wandb:
            wandb.init(
                project=f'FGVC-{args.dataset_name}-{args.num_classes}',
                name=log_time,
                config=tracker_config,
                dir=log_root,
                mode='offline',
