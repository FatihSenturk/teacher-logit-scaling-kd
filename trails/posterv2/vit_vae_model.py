...
class VICHClassifier(nn.Module):
    """
    Lightweight class-level variational classifier head.

    This predicts mu/logvar directly in class space:
    Linear(feature_dim, num_classes). It intentionally avoids
    Linear(d, d) projections and decoder/reconstruction layers.
    """
    def __init__(
        self,
        in_features,
        ...
        logvar_min=-10.0,
        logvar_max=10.0,
        init_logvar_bias=-5.0,
    ):
        super(VICHClassifier, self).__init__()
        ...
        self.fc_mu = nn.Linear(self.in_features, self.out_features)
        self.fc_logvar = nn.Linear(self.in_features, self.out_features)
    ...
    def reset_parameters(self):
        ...
        nn.init.constant_(self.fc_logvar.bias, self.init_logvar_bias)
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=14, patch_size=14, in_c=147, num_classes=7,
                 ...
                 act_layer=None,vae=False,vich=False,vich_use_sampling=True,
                 vich_logvar_min=-10.0,vich_logvar_max=10.0,vich_init_logvar_bias=-5.0):
        """
        Args:
        ...
        if self.vich:
            self.head_vich.reset_parameters()
