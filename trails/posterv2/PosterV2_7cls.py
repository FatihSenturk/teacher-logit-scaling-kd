...
def _resolve_pretrained_path(filename):
    candidates = []
    configured_dir = os.environ.get("POSTER_VAR_PRETRAINED_DIR")
    if configured_dir:
    ...
    raise FileNotFoundError(f"POSTERv2 pretrained file not found: {filename}. Checked: {checked}")

def load_pretrained_weights(model, checkpoint):
    import collections

    def _remap_ir50_key(key):
        """
        Convert original ArcFace IR50 keys (body.N.*) to this repo's split bodies:
        - body.0..2   -> body1.0..2
        - body.3..6   -> body2.0..3
        - body.7..20  -> body3.0..13
        - body.21..23 -> dropped (body4 not used here)
        """
        ...
        tail = ".".join(parts[2:])
        if 0 <= idx <= 2:
            return f"body1.{idx}.{tail}"
        if 3 <= idx <= 6:
            return f"body2.{idx - 3}.{tail}"
        if 7 <= idx <= 20:
            return f"body3.{idx - 7}.{tail}"
        # body4 is intentionally omitted in this architecture
        return None
...
class pyramid_trans_expr2(nn.Module):
    def __init__(
        ...
        face_landback_checkpoint = torch.load(
            _resolve_pretrained_path("mobilefacenet_model_best.pth.tar"),
        ...
        self.VIT = VisionTransformer(
            ...
            vae=self.vae,
            vich=self.vich,
            vich_use_sampling=vich_use_sampling,
        ...
        ir_checkpoint = torch.load(
            _resolve_pretrained_path("ir50.pth"),
            map_location=lambda storage, loc: storage,
        )
